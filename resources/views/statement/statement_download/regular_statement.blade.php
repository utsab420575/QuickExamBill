        ...
        @foreach($assigns_order_7_e as $i => $assign)
            ...
            <tr>
                ...
                <td style="text-align:left;">
                    {{ $t->user->name ?? 'N/A' }},
                    {{ $t->designation->designation ?? '' }},
                    {{ $t->department->fullname ?? '' }},
                    {{ $assign?->university?->short_name ?? 'DUET' }}, {{ $assign?->university?->city ?? 'Gazipur' }}
                </td>
                @php
                    $students = (float) ($assign->total_students ?? 0);  // no (int) here
                @endphp
        ...
        @foreach($assigns_order_6_a as $row)
            ...
            <tr>
                ...
                <td style="text-align:left;">
                    ...
                    @if($dept), {{ $dept }}@endif, {{ $row?->university?->short_name ?? 'DUET' }}, {{ $row?->university?->city ?? 'Gazipur' }}
